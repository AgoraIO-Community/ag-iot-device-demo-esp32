...
esp_err_t esp_blufi_host_and_cb_init(esp_blufi_callbacks_t *example_callbacks)
{
    ...
    /* register BLE GAP (handle the BLE GAP info) */
...
esp_err_t esp_blufi_host_and_cb_deinit(void)
{
    ...
    /* deregister BLE GAP (handle the BLE GAP info) */
...
esp_err_t esp_blufi_host_and_cb_deinit(void)
{
    ...
    ret = esp_blufi_host_deinit();
    if (ret) {
        BLUFI_ERROR("%s uninitialise host failed: %s\n", __func__, esp_err_to_name(ret));
        return ret;
    }    
}
